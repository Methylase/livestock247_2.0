<div class="wrapper-blog-post-hero">
    <div class="wrapper-blog-post-content text-left">
        <h5 class="text-left blog-post-subtitle">
           ...
           <?=  $blog_post->link_post_title ?>
        </h5>
        <h2 class="text-left blog-post-main-title"><?=  $blog_post->post_title ?> </h2>
        <p><span class="posted-date"><?= Modules::run('blog/get_created_date',$blog_post->trongate_user_id) ?></span><span class="dot"> • </span><span class="last-read-time show<?=  $blog_post->id ?>" id="<?=  $blog_post->id ?>"> <?= Modules::run('blog/last_read_time',$blog_post->trongate_user_id) ?></span></p>
        <div class="blog-post-content">
            <img src="<?= BASE_URL.'blog_post_upload/'.$blog_post->post_image ?>" class=" w-100 img-fluid img-blog-post">
            <p class="blog-post-content">
              <?= ucfirst($blog_post->post_description) ?>
            </p>
            <div class="blog-post-author d-flex justify-content-between">
                <div class="author-content d-flex align-items-center justify-content-center">
                    <span class="author-tag"><?= Modules::run('profile/get_user_tag',$blog_post->trongate_user_id) ?></span>
                    <span class="author-details mt-3 ml-3">
                        <h6 class="author-name"><?= Modules::run('profile/get_user_fullname',$blog_post->trongate_user_id) ?></h6>
                        <p><span class="author-role"> <?= Modules::run('profile/get_user_designated_position',$blog_post->trongate_user_id) ?></span></p>
                    </span>
                </div> 
                <div>
                    <p>
        ...
        <div class="related-blog-post-article">
            ...
            <?php if(isset($related_post) && $related_post !=null){ ?>
            ...
            ?>
                ...
                <?php foreach($related_post as $post){ ?>
                    <?php $blog_result .= '<div class="col-lg-4 col-md-4">
                                                <div class="col-article-content blog-post" id="'.$post->id.'">
                                                    ...
                                                    <div class="author d-flex align-items-start">
                                                        <div class="author-content d-flex align-items-center justify-content-center">
                                                            ...
                                                            <span class="author-details mt-3 ml-3">
                                                                ...
                                                                <p><span class="date-posted">'.Modules::run('blog/get_created_date',$post->trongate_user_id).'</span><span class="last-read-time">'.Modules::run('blog/last_read_time',$post->trongate_user_id).'</span></p>
                                                            </span>
                                                        </div> 
                                                    </div>                    
                                                </div>
        ...
        <?php } ?>   
         ...
         <div class="text-center mt-3">
            <a href="<?= BASE_URL.'blog?page=1'?>" class="btn btn-primary">See all articles</a>
         </div>
    </div>
</div>